"""
These codes are originally written by the authors of DeepGOZero 
and the original source is https://github.com/bio-ontology-research-group/deepgozero

We use these codes to ensure consistency in the comparison of the methods 
We gratefuly acknowledge the contributions of Kulmanov et al.
"""
...
class Ontology(object):
    ...
    def __init__(self, filename='data/go.obo', with_rels=False):
        ...
        self.ic = None
        self.ic_norm = 0.0
        self.include_height(BIOLOGICAL_PROCESS,0,NAMESPACES['bp'])
        self.include_height(MOLECULAR_FUNCTION,0,NAMESPACES['mf'])
        self.include_height(CELLULAR_COMPONENT,0,NAMESPACES['cc'])

    def has_term(self, term_id):
        return term_id in self.ont

    def get_depth(self, term_id):
        return self.ont[term_id]['depth']
    ...
    def include_height(self, node, level,namespace):
        if self.ont[node]['depth'] == -1:
            self.ont[node]['depth'] = level

        for child_node in self.ont[node]['children']:
            if self.ont[child_node]['namespace']!=namespace:
                continue
            if self.ont[child_node]['depth'] == -1:
                self.ont[child_node]['depth'] = level + 1
                self.include_height(child_node, level+1,namespace)
